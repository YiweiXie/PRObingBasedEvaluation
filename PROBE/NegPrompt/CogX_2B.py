...
def parse_args():
    ...
    parser.add_argument("--neg_prompt", type=str, default="", help="negprompt for CI")
...
class TextualInversionDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        example["neg_ids"] = self.tokenizer(
            neg_text,
            padding="max_length",
            truncation=True,
            max_length=self.tokenizer.model_max_length,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(pipe.text_encoder):
                ...
                bsz = latents.shape[0]
                # Sample a random timestep for each video in the batch
                timesteps = torch.randint(0, pipe.scheduler.config.num_train_timesteps, (bsz,), device=latents.device)
                timesteps = timesteps.long()

                # Get the text embedding for conditioning
                target_prompt = pipe.text_encoder(batch["input_ids"])[0].to(dtype=weight_dtype)
                neg_prompt = pipe.text_encoder(batch["neg_ids"])[0].to(dtype=weight_dtype)
                ...
                with torch.no_grad():
                    neg_pred = pipe.transformer(
                        ...
                        encoder_hidden_states=neg_prompt,
                        timestep=timesteps
                    ).sample
                
                target_pred = pipe.transformer(
                    hidden_states=noisy_latents,
                    encoder_hidden_states=target_prompt,
                    timestep=timesteps
                ).sample

                model_pred = neg_pred + 6.0 * (target_pred - neg_pred)
