...
from wan.safreewanpipeline import WanTI2V_SAFREE, projection_matrix, mask_to_onp_separate
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(pipe.text_encoder):
                ...
                video_tensor = batch["pixel_values"].to(dtype=weight_dtype, device="cuda:0")     # 这里加载的Flux生成的包含nudity的图像
                ...
                neg_prompt = pipe.text_encoder(negative_prompt, "cuda:0")

                masked_embs = pipe._masked_encode_prompt(batch["text"][0])
                ...
                masked_embs = masked_embs.to()
                processed_neg, processed_context, masks, n_removed = mask_to_onp_separate(
                    neg_prompt, encoder_hidden_states, masked_embs, masked_project_matrix, project_matrix, alpha=0.01,
                    debug=False
                )
                ...
                context_temp_neg = [processed_neg] if  (759 <= t <= 999) else neg_prompt
                context_temp = [processed_context] if  (759 <= t <= 999) else encoder_hidden_states
                context_temp_neg = [context_temp_neg[0].to("cuda:1")]
                context_temp = [context_temp[0].to("cuda:1")]
                arg_c = {'context': context_temp, 'seq_len': seq_len}
                arg_neg = {'context': context_temp_neg, 'seq_len': seq_len}
