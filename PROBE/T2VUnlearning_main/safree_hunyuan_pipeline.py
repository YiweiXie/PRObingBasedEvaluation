...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        diffusers.     model_id, subfolder="transformer", torch_dtype=torch.bfloat16
        diffusers. )
        ...
        >>> output = pipe(
        diffusers.     prompt="A cat walks on the grass, realistic",
        diffusers.     height=320,
        diffusers.     width=512,
        diffusers.     num_frames=61,
        diffusers.     num_inference_steps=30,
        diffusers. ).frames[0]
...
def mask_to_onp(input_embeddings, p_emb, masked_input_subspace_projection, concept_subspace_projection, 
                alpha=0., max_length=226, debug=False):
    """
    ie = [1, max_length, latent_dim] <-- only pos
    ms = [latent_dim, latent_dim]
    cs = [latent_dim, latent_dim]
    """
    ie = input_embeddings
    ...
    new_embeddings = merged_text_e.unsqueeze(0)
...
class HunyuanVideoPipeline(DiffusionPipeline, HunyuanVideoLoraLoaderMixin):
    ...
    def _encode_negative_prompt_llama(
        self,
        prompt: Union[str, List[str]],
        prompt_template: Dict[str, Any],
        num_videos_per_prompt: int = 1,
    ...
    def _get_clip_prompt_embeds(
        ...
        embeds = self.text_encoder_2(text_input_ids.to(device), output_hidden_states=False)
        unpooled = embeds.last_hidden_state
        prompt_embeds = embeds.pooler_output

        # duplicate text embeddings for each generation per prompt, using mps friendly method
        prompt_embeds = prompt_embeds.repeat(1, num_videos_per_prompt)
        prompt_embeds = prompt_embeds.view(batch_size * num_videos_per_prompt, -1)

        return unpooled, prompt_embeds
    
    def _encode_negative_prompt_clip(
        self,
        prompt: Union[str, List[str]],
        num_videos_per_prompt: int = 1,
        device: Optional[torch.device] = None,
    ...
    def encode_prompt(
        ...
        if pooled_prompt_embeds is None:
            if prompt_2 is None:
                prompt_2 = prompt
            unpooled_clip, pooled_prompt_embeds = self._get_clip_prompt_embeds(
                prompt,
                num_videos_per_prompt,
                device=device,
                dtype=dtype,
        ...
        return prompt_embeds, pooled_prompt_embeds, prompt_attention_mask, unpooled_clip
    
    def _masked_encode_prompt_llama(self, 
        prompt: Union[str, List[str]],
        prompt_template: Dict[str, Any],
        device: Optional[torch.device] = None,
        dtype: Optional[torch.dtype] = None,
        ...
        untruncated_ids = untruncated_ids[:, crop_start:]
        print('untruncated_ids', untruncated_ids.shape)
        n_real_tokens = untruncated_ids.shape[1] -1 #special token at the end

        if untruncated_ids.shape[1] > max_sequence_length:
            untruncated_ids = untruncated_ids[:, :max_sequence_length]
            n_real_tokens = max_sequence_length -1
    ...
    def _masked_encode_prompt_clip(
        self,
        prompt: Union[str, List[str]],
        device: Optional[torch.device] = None,
        dtype: Optional[torch.dtype] = None,
        ...
        if untruncated_ids.shape[1] > max_sequence_length:
            untruncated_ids = untruncated_ids[:, :max_sequence_length]
            n_real_tokens = max_sequence_length -2
        masked_ids = untruncated_ids.repeat(n_real_tokens, 1)

        for i in range(n_real_tokens):
            masked_ids[i, i+1] = 0

        masked_embeddings = self.text_encoder_2(
            masked_ids.to(device),
            attention_mask=None,
    ...
    def __call__(
        ...
        do_true_cfg = False
        #when set to True the generation output is a total mess
        ...
        negative_safree = CONCEPT_DICT[concept]
        negative_prompt = ", ".join(negative_safree)

        masked_embs_clip = self._masked_encode_prompt_clip(prompt)
        masked_embs_llama = self._masked_encode_prompt_llama(prompt,prompt_template=DEFAULT_PROMPT_TEMPLATE)

        masked_project_matrix_clip = projection_matrix(masked_embs_clip.T) 
        masked_project_matrix_llama = projection_matrix(masked_embs_llama.T)

        neg2_text_embeddings_clip = self._encode_negative_prompt_clip(negative_safree)
        neg2_text_embeddings_llama = self._encode_negative_prompt_llama(negative_safree,prompt_template=DEFAULT_PROMPT_TEMPLATE)

        project_matrix_clip = projection_matrix(neg2_text_embeddings_clip.T)
        project_matrix_llama = projection_matrix(neg2_text_embeddings_llama.T)

        # 3. Encode input prompt
        transformer_dtype = self.transformer.dtype
        prompt_embeds, pooled_prompt_embeds, prompt_attention_mask, unpooled_clip = self.encode_prompt(
            prompt=prompt,
            prompt_2=prompt_2,
        ...
        rescaled_pooled_embeddings, sp_vector, inv_vector, n_removed = mask_to_onp(unpooled_clip, masked_embs_clip,
                                                                masked_project_matrix_clip, 
                                                                project_matrix_clip,
                                                                max_length=77,
                                                                alpha=0.01,
                                                                debug=False)
        #averaging over token dim to get a pooled representation
        rescaled_pooled_embeddings = rescaled_pooled_embeddings.mean(dim=1) 
        rescaled_prompt_embeddings, sp_vector, inv_vector, n_removed = mask_to_onp(prompt_embeds, masked_embs_llama,
                                                                masked_project_matrix_llama, 
                                                                project_matrix_llama,
                                                                max_length=256,
                                                                alpha=0.01,
                                                                debug=False)

        if do_true_cfg: #classifier-free guidance mess with the safree generation process
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                #we followed the cogvideox safree version. It seems that increasing the steps improves the effectiveness of erasure
                llama_embeddings = rescaled_prompt_embeddings.to(transformer_dtype) if  (0 <= i <= 12) else prompt_embeds
                clip_embeddings = rescaled_pooled_embeddings.to(transformer_dtype) if (0 <= i <= 12) else pooled_prompt_embeds

                noise_pred = self.transformer(
                    hidden_states=latent_model_input,
                    timestep=timestep,
                    encoder_hidden_states=llama_embeddings,
