...
sys.path.append("../T2VUnlearning_main")
from receler.erasers.wan_erasers import inject_eraser
...
class TextualInversionDataset_I2P(Dataset):
    ...
    def __getitem__(self, i):
        ...
        real_i = self.valid_indices[i % len(self.valid_indices)]
        index_str = str(real_i)
        frame_paths = glob.glob(os.path.join(self.data_root, f"{index_str}_*.png"))     # cogvideo
...
def main():
    ...
    eraser_path = args.esd_checkpoint
    eraser_rank = 128
    if eraser_path:
        ...
        # inject erasers into pretrained Wan2.2
        inject_eraser(pipe.model, eraser_ckpt=torch.load(eraser_ckpt_path, map_location='cpu'), 
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(pipe.text_encoder):
                ...
                neg_prompt = pipe.text_encoder([n_prompt], "cuda:0")
