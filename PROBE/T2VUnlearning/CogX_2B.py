...
from receler.erasers.cogvideo_erasers import inject_eraser
import diffusers
...
def parse_args():
    ...
    parser.add_argument("--neg_prompt", type=str, default="", help="negative prompt")
    ...
    def __init__(
        ...
        for item in self.metadata:
            ...
            if index_str not in seen_indices:
                ...
                self.captions[index_str] = "{} " + item["prompt"]
...
def main():
    ...
    if args.esd_checkpoint != "":        
        eraser_ckpt_path = os.path.join(args.esd_checkpoint, f'eraser_weights.pt')
        eraser_config_path = os.path.join(args.esd_checkpoint, f'eraser_config.json')
        with open(eraser_config_path) as f:
            eraser_config = json.load(f)
        # inject erasers into pretrained cogvideo
        inject_eraser(pipe.transformer, eraser_ckpt=torch.load(eraser_ckpt_path, map_location='cpu'), eraser_rank=128)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(pipe.text_encoder):
                ...
                encoder_hidden_states = pipe.text_encoder(batch["input_ids"])[0].to(dtype=weight_dtype, device="cuda:0")
                ...
                model_pred = pipe.transformer(hidden_states=noisy_latents,encoder_hidden_states=encoder_hidden_states, timestep=timesteps).sample
